...
@export_enum("4LongPlatform", "8LongPlatform", "12LongPlatform", "16LongPlatform", "20LongPlatform", "4LongDeathPlatform", "8LongDeathPlatform", "12LongDeathPlatform", "16LongDeathPlatform", "20LongDeathPlatform") var sprite: String
@export var shape: Shape2D
@export var just_detect: bool
@export_enum("Floor","Death") var group: String
...
func _ready() -> void:
	if just_detect == true:
		...
		print("Just Detect Is on")
	elif just_detect == false:
		collision_shape.disabled = false
		detector_shape.disabled = true
	detector_area.add_to_group(group)
	...
	var detector_shape_track_index = -1
	...
	if move_anim:
		...
		for detectr_colsn_shape_track in range(move_anim.get_track_count()): #Looks through the current list of tracks that is in the move animation
			if move_anim.track_get_path(detectr_colsn_shape_track) == NodePath("Detector/CollisionShape2D:shape"): #Specificies what track we are looking for. In this case we are looking for the track that is manipulating the root node's position
				detector_shape_track_index = detectr_colsn_shape_track #sets the track index to the specified track
		...
		if root_track_index != -1: # this just checks to see if we ever found the track that we pointed to at line 14 if not it will just return nothign
			move_anim.track_insert_key(root_track_index, 0.0, starting_pos) # Creates a key that begins at the preset starting position
			move_anim.track_insert_key(root_track_index, 1.0, ending_pos) # Creates a key that ends at the preset ending position
		...
		if detector_shape_track_index != -1: # this just checks to see if we ever found the track that we pointed to at line 14 if not it will just return nothign
			move_anim.track_insert_key(detector_shape_track_index, 0.0, detector_shape.shape) # Creates a key that sets the collision shape of the current animation
		elif detector_shape_track_index == -1:
			print("Detector Shape not found")
			return
